...
def receivedMsg():
    global gameOver, flashNumberList
    
    numbers = [str(i) for i in range(1, 91)]  # Create a list of numbers from 1 to 90
    ...
    while True:
        try:
            ...
            if not gameOver:
                if chunk in numbers and chunk not in flashNumberList:
                    flashNumberList.append(int(chunk))
                    flashLabel.create_text(150, 25, text=chunk, font=("Helvetica", 12))
                elif "wins the game" in chunk:
                    gameOver = True
                    flashLabel.create_text(150, 25, text="Game Over!", font=("Helvetica", 12))
        except:
